...
export default function UiToggle({ checked = false, onChange, label, disabled = false }) {
  return (
    <label className={`inline-flex items-center ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
      <span className={`relative inline-flex shrink-0 h-6 w-11 rounded-full border-2 border-transparent transition-colors duration-200 ${checked ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-600'}`} onClick={() => !disabled && onChange?.(!checked)} role="switch" aria-checked={checked}>
