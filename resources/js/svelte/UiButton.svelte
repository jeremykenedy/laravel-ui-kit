<script>
  ...
  $: classes = `inline-flex items-center justify-center font-medium rounded-lg transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${sizes[size] || sizes.md} ${variants[variant] || variants.primary} ${block ? 'w-full' : ''} ${disabled || loading ? 'opacity-50 cursor-not-allowed' : ''}`
</script>
