...
class Icon extends Component implements ComponentContract
{
    /**
     * Stroked 24x24 outline paths, shared by every CSS framework.
     *
     * The markup is framework agnostic on purpose: only sizing differs per framework,
     * so the geometry lives here instead of being duplicated in three view files.
     *
     * @var array<string, string>
     */
    protected const PATHS = [
        'check'         => '<path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>',
        'save'          => '<path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>',
        ...
        'refresh'       => '<path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992V4.356m0 4.992l-3.181-3.183a8.25 8.25 0 00-13.803 3.7M4.031 9.865v4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7"/>',
    ...
    protected const FALLBACK_PATH = '<path stroke-linecap="round" stroke-linejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z"/>';
    ...
    /**
     * Tailwind sizing utilities for the rendered SVG.
     */
    ...
    /**
     * Pixel size for frameworks without sizing utilities, such as Bootstrap.
     */
    ...
    {
        $set = config('ui-kit.icons', 'lucide');

        return is_string($set) && $set !== '' ? $set : 'lucide';
    }

    public function resolvedClass(): string
    {
        return match ($this->iconSet()) {
    ...
    /**
     * Whether the configured icon set renders through a webfont class rather than inline SVG.
     */
    public function usesIconFont(): bool
    {
        return $this->iconSet() === 'fontawesome';
    }

    /**
     * The inline SVG body for this icon, falling back to a neutral "unknown" glyph.
     */
    public function path(): string
    {
        return self::PATHS[$this->name] ?? self::FALLBACK_PATH;
    }

    /**
     * Whether this icon name has dedicated geometry.
     */
    public function hasPath(): bool
    {
        return isset(self::PATHS[$this->name]);
    }

    /**
     * Every icon name this package ships geometry for.
     *
     * @return list<string>
     */
